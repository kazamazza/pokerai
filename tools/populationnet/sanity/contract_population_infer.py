...
def run_contract(ckpt: Path, sidecar: Path, req_path: Path, batch_size: int = 256) -> None:
    infer = PopulationNetInfer.from_checkpoint(
    ...
    for i in range(0, len(rows), batch_size):
        ...
        p = infer.predict_proba(batch)  # [B,3] = [FOLD,CALL,RAISE] tendencies per cell context
        p_np = p.detach().cpu().numpy()
        for j in range(p_np.shape[0]):
            shown += 1
            probs = p_np[j].tolist()
            print("—" * 56)
            print(f"[{shown:03d}] population tendencies")
            print(f" FOLD={probs[0]:.3f}  CALL={probs[1]:.3f}  RAISE={probs[2]:.3f}")
...
def main():
    ap = argparse.ArgumentParser(description="Contract-style PopulationNet inference (no preprocessing).")
    ...
    ap.add_argument("--requests", type=Path, default=Path("data/samples/population_requests.jsonl"),
