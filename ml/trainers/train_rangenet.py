...
from ml.datasets.rangenet import rangenet_collate_fn, RangeNetDatasetParquet
from ml.datasets.utils_dataset import stratified_indices
from ml.utils.config import load_model_config
from ml.models.rangenet import RangeNetLit
...
def run_train(cfg: Mapping[str, Any]) -> str:
    ...
    if not x_cols:
        raise ValueError("dataset.x_cols must be provided for RangeNet")
    min_weight = _get(cfg, "dataset.min_weight", None)

    ds = RangeNetDatasetParquet(
    ...
    stratify_keys = _get(cfg, "dataset.stratify_keys", None)  # e.g. ["street"] for postflop
    ...
    # -------- Callbacks / Logger --------
    ckpt_dir = Path(_get(cfg, "train.checkpoints_dir", "checkpoints/rangenet"))
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    ...
    if logger_name == "tensorboard":
        logger = TensorBoardLogger(save_dir=_get(cfg, "logging.tb_log_dir", "logs/tb"), name="rangenet")
    else:
        logger = False

    ckpt_cb = pl.callbacks.ModelCheckpoint(
        dirpath=str(ckpt_dir),
        filename="rangenet-{epoch:02d}-{" + monitor + ":.4f}",
...
if __name__ == "__main__":
    ...
    ap.add_argument("--config", type=str, default="rangenet/preflop/dev",
                    help="Model name or YAML path (resolved by load_model_config). "
                         "Examples: rangenet/preflop/dev  or  rangenet/postflop/dev")
    # light overrides for train.*
