...
def run_postflop_sweep(cfg: dict):
    base_dir = Path(cfg.get("train", {}).get("checkpoints_dir_facing", "checkpoints/postflop_policy_facing"))
    res = run_sweep(
        base_cfg=cfg,
        sweep=cfg.get("sweep", {}),
        run_fn=run_train_postflop_facing,
    ...
    if res["best"].get("ckpt"):
        finalize_best_artifacts(Path(res["best"]["ckpt"]), base_dir)
    return res

def run_train_postflop_facing(cfg: Mapping[str, Any]) -> str:
    ...
    # --- schema from YAML (same keys you used for root) ---
    ...
    # Ensure board_cluster_id is present if you use it as a categorical
    ...
    if stratify_keys:
        train_idx, val_idx = stratified_indices(ds._df, stratify_keys, train_frac, int(_get(cfg,"train.seed",42)))
    else:
        n = len(ds); cut = int(n * train_frac); idx = list(range(n))
    ...
    train_dl = DataLoader(
        ...
        num_workers=int(_get(cfg,"train.num_workers",0)), pin_memory=True,
        collate_fn=postflop_policy_facing_collate_fn,
    )
    val_dl = DataLoader(
        val_ds, batch_size=min(batch, len(val_ds)), shuffle=False,
        num_workers=int(_get(cfg,"train.num_workers",0)), pin_memory=True,
        collate_fn=postflop_policy_facing_collate_fn,
    )
    ...
    logger = TensorBoardLogger(
        ...
        name="postflop_policy_facing"
    ) if _get(cfg,"logging.logger","tensorboard") == "tensorboard" else False

    ckpt_cb  = pl.callbacks.ModelCheckpoint(
        dirpath=str(ckpt_dir),
        filename="postflop_policy_facing-{epoch:02d}-{" + monitor + ":.4f}",
