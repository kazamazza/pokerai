...
def mask_root(actions: Sequence[str], *, actor: str, bet_menu: Optional[Sequence[float]]) -> torch.Tensor:
    ...
    if bet_menu:
        want = set()
        def has(x: float) -> bool: return any(abs(float(s)-x) < 1e-3 for s in bet_menu)
        ...
        for b in {"BET_25","BET_33","BET_50","BET_66","BET_75","BET_100"}:
            if b not in want: legal.discard(b)
        if "DONK_33" not in want: legal.discard("DONK_33")
    m = torch.zeros(len(actions), dtype=torch.float32)
    for i,a in enumerate(actions):
        if a in legal: m[i]=1.0
    if m.sum().item()==0: m.fill_(1.0)
    return m
